...
// Función para recuperar valor: POST > COOKIE > COOKIE (sin POST) > DB > NULL
if (! function_exists('cookie_revisar')) {
    /**
     * Gestiona automáticamente la persistencia de un campo de formulario (principalmente filtros).
     *
     * @param string $key Nombre del campo
     * @param mixed $default Valor por defecto
     * @return mixed
     */
    function cookie_revisar($key, $default = null)
    {
        $CI = &get_instance();

        // 1. Si viene por POST, es la nueva verdad.
...
// Función avanzada para inputs RECORDABLES (Edición/Creación)
if (! function_exists('input_recordar')) {
    /**
     * Mantiene el valor de un input priorizando POST > COOKIE > DB > NULL
     *
     * @param string $campo Nombre del input
     * @param mixed $valor_db Valor que viene de la base de datos (si es edición)
     * @param string $id_unico Identificador del formulario (para evitar choques entre formularios)
     * @return mixed
     */
    function input_recordar($campo, $valor_db = null, $id_unico = 'nuevo')
    {
        $CI = &get_instance();

        // 1. Si hay POST (intento fallido de guardar o recarga), guardamos cookie y retornamos valor
        if ($CI->input->post($campo) !== null) {
            $valor = $CI->input->post($campo);
            $cookie = array(
                'name'   => "draft_{$id_unico}_{$campo}",
                'value'  => $valor,
                'expire' => '7200', // 2 horas
                'path'   => '/'
            );
        ...
        // 2. Si no hay POST, revisar si existe una COOKIE guardada (Borrador)
        // IMPORTANTE: Solo usamos la cookie si $valor_db es NULL (estamos creando) O si queremos priorizar borrador sobre DB.
        // En este caso, para no sobreescribir la DB con un borrador viejo accidentalmente, 
        // usaremos la cookie solo si existe y NO hay valor DB, O si es explícitamente un form de creación.
        // Pero para "Edición interrumpida", queremos ver el borrador. 
        // Logica: Si hay cookie, la mostramos. El usuario debe decidir si limpiar o no (normalmente al guardar exitosamente se limpia).

        $cookie_val = $CI->input->cookie("draft_{$id_unico}_{$campo}");
        if ($cookie_val !== null) {
            return $cookie_val;
        }

        // 3. Si no hay nada, usar el valor de la Base de Datos
        return $valor_db;
    }
}

if (! function_exists('borrar_drafts')) {
    /**
     * Borra las cookies temporales de un formulario exitoso.
     *
     * @param array $campos_array Arreglo con los nombres de los inputs.
     * @param string $id_unico Identificador del formulario usado en input_recordar.
     */
    function borrar_drafts($campos_array, $id_unico = 'nuevo')
    {
        $CI = &get_instance();
        foreach ($campos_array as $campo) {
            delete_cookie("draft_{$id_unico}_{$campo}");
        }
    }
}
